...
def load_pdf(file_path):
    st.write("UI: Starting to load PDF...")
    ...
    st.write(f"UI: Found {len(elements)} elements")

    text_elements = [element.text for element in elements if element.category not in ["Image", "Table"]]
    st.write(f"UI: Found {len(text_elements)} text elements")
    for file in os.listdir(figures_directory):
        st.write(f"Console: Processing figure: {file}")
